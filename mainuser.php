...
if (str=="")
  {
  document.getElementById("sel_program2").innerHTML="";
  return;
  }
if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
    document.getElementById("sel_program2").innerHTML=xmlhttp.responseText;
    }
  }
 ...
 <script>
	 ...
	 <script>
		...
		function changecapacity(){
			...
			var data = {'cpLvl' : lvl_capacity,
						'choiceToAddLevelCo' : lvl_code };
				$.ajax({
					type: "POST",
	 ...
	 <script>
		function edit_cu(str) {
			...
			function edit_sec(str) {
			var id = "form.section".concat(str);
			var temp = "#form-content-sec".concat(str);
			var ty = "#thanks-sec".concat(str);
		
			var data = $(id).serialize();
			
				$.ajax({
					type: "POST",
					url: "includes/editSec.php", // 
					data: data,
					success: function(msg){
						$(ty).html(msg);
						$(temp).modal('hide');	
			...
			function edit_pro(str) {
			var id = "form.pro".concat(str);
			var temp = "#form-pro".concat(str);
			
			var data = $(id).serialize();
			
				$.ajax({
					type: "POST",
					url: "includes/editPro.php", // 
					data: data,
					success: function(msg){
	...
	<script>
		...
		<script>
		$(document).ready(function () {
			$("input#add_prof").click(function(){
				$.ajax({
					type: "POST",
					url: "includes/addProf.php", // 
					data: $('form.add_prof').serialize(),
					success: function(msg){
						$("#thanks-prof").html(msg)
						$("#form-prof").modal('hide');	
					},
					error: function(){
						alert("failure");
					}
	...
	function add_cu(str) {
			...
			var data = $(id).serialize();
				...
				$.ajax({
					type: "POST",
					url: "includes/addProf_cou.php",  
					data: data,
					success: function(msg){
						$("#thanks-prof_cou").html(msg)
						$("#form-prof-cou").modal('hide');	
					},
					error: function(){
						alert("failure");
					}
...
<!-- 
			...
			<div class="modal-body">
				...
				<select id="trmCodeSelect" name="trmCodeSelect" class="form-control" >
					...
					<select id="trmCodeSelect" name="level" class="form-control" >
							<?php 
								require("includes/selectLevel.php"); 
							?>
						</select><br>
  ...
  <form  class="editCourseSectionForm"  > 
	<!-- Add new section--> 
	...
	<div class="form-group">
	<div class="col-xs-3">
	<input type="submit"  id="AddSec" class="btn btn-primary " value="Add New Section" />
	</div>
	</div>
	<div id="thankss"><p></p></div>
	</form>
		...
		*/ ?>
		...
		<div class="tabContent" id="tab7">
			...
			<div class="col-xs-3">
				...
				<!-- Add new Prof Button -->
				<div id="form-prof" class="modal hide fade in" style="display: none;">
			<div class="modal-header">
				<a class="close" data-dismiss="modal">×</a>
				<h3>Add new Course</h3>
			</div>
			<div class="modal-body">
				<form class="add_prof" name="program">
					<label class="label" for="code">Professor Code</label><br>
					<input type="text" name="code" class="input-xlarge"><br>
					<label class="label" for="version">First name</label><br>
					...
					<label class="label" for="level">Professor Constraint</label><br>
					<input type="text" name="constraint" class="input-xlarge"><br>
					<label class="label" for="level">Phone Number</label><br>
					<input type="text" name="number" class="input-xlarge"><br>
					<label class="label" for="level">Email</label><br>
					<input type="text" name="email" class="input-xlarge"><br>
					<label class="label" for="level">Office Number</label><br>
					<input type="text" name="ofc_number" class="input-xlarge"><br>
					
				</form>
			</div>
			<div class="modal-footer">
				<input class="btn btn-success" type="submit" value="Add" id="add_prof">
				<a href="#" class="btn" data-dismiss="modal">Cancel</a>
			</div>
		</div>
	
				<div id="thanks-prof"><p></p></div>
				<a data-toggle="modal" href="#form-prof" class="btn btn-primary">Add new Professor</a>
				<br>
