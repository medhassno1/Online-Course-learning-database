...
<!--  HTML starts here -->
		...
		<?php foreach($rows as $row): $lvl_capacity = $row['capacity']; ?>
				...
				<label class="label-control col-xs-2">Capacity for the Level       	:</label><div class="col-xs-3">
					<input type="text" class="form-control" name="cpLvl"  value="<?php echo htmlentities($row['capacity'], ENT_QUOTES, 'UTF-8'); ?>" /> </div>
			
			
		<?php endforeach; ?>
		
	<div class="col-xs-3">
						<button type="submit" form="editCourseForm" id = "editcapa" class="btn btn-primary " value="SaveCapacity">Save</button>
					</div>
					</form>
				<br>
<?php
			...
			<div class="modal-body">
				<form class="contact<?php echo $i; ?>" name="program">
				 ...
				 <?php foreach($rowss as $rowe): ?>
					...
					<input type="text" name="level" class="input-xlarge" value="<?php echo $rowe['Level']; ?>"><br>
